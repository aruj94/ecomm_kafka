...
export const findCart = async (id: number): Promise<CartWithLineItems> => {
...
export const findCartByProductId = async ( customerId: number, productId: number ) : Promise<CartLineItem> => {
    const cart = await DB.query.carts.findFirst({
